...
const live={follow:null, sub:null, bits:null, don:null, raid:null};
...
/* ─── Taille unique carrée ─── */
// La taille se règle directement dans OBS (redimensionnement libre)
// On conserve 480x480 comme base
const CAM_SIZE={w:480,cs:56,bw:3};
...
function renderGoalBar(mode){
  ...
  if(mode==='follow'){
    const target=num(fd('followGoalTarget'),0); if(!target){switchGoalMode('sub');return;}
    ...
    if(icon)icon.textContent='♥'; if(lbl)lbl.textContent='Objectif Followers';
    if(counter)counter.textContent=`${fmtN(cur)} / ${fmtN(target)}`;
    if(fill)fill.style.width=p+'%'; if(pct)pct.textContent=p+'%';
  } else {
    const target=num(fd('subGoalTarget'),0); if(!target){switchGoalMode('follow');return;}
    const cur=num(fd('subGoalCurrent'),0);
    const p=Math.min(100,Math.round(cur/target*100));
    wrap.className='show sub';
    if(icon)icon.textContent='★'; if(lbl)lbl.textContent='Objectif Abonnements';
...
function switchGoalMode(forceTo){
  const hF=hasFollowGoal(),hS=hasSubGoal();
  if(!hF&&!hS){$('goal-wrap')?.classList.remove('show');$('cam-root')?.classList.remove('has-goal');return;}
  if(!hF)goalMode='sub'; else if(!hS)goalMode='follow';
...
function startGoalSwitcher(){
  ...
  if(hasFollowGoal()&&hasSubGoal()){
    goalTimer=setInterval(()=>switchGoalMode(),num(fd('goalSwitchInterval'),8)*1000);
  }
}
...
function buildSocItems(){
  ...
  for(let i=1;i<=6;i++){
    ...
    const p=fd(`social${i}Platform`)||'twitch', n=fd(`social${i}Name`)||'';
...
function applyStatItem(item){
  ...
  icon.style.borderColor=item.color; icon.style.boxShadow=`0 0 10px ${item.color}`;
  lbl.textContent=item.label; lbl.style.color=item.color;
  val.textContent=item.value; val.style.color=item.color; val.style.textShadow=`0 0 10px ${item.color}`;
}
...
function swap(side){
  const elId=side==='stat'?'bt-stat':'bt-soc';
  const el=$(elId); if(!el) return;
  el.classList.add('fout');
  setTimeout(()=>{
    if(side==='stat'){si=(si+1)%statItems.length;applyStatItem(statItems[si]);updateDots('stat-dots',si,statItems.length);}
    else{if(!socItems.length)return;ri=(ri+1)%socItems.length;applySocItem(socItems[ri]);updateDots('soc-dots',ri,socItems.length);}
...
function getIntensity(t){
  ...
  if(g==='off')return'off';
  if(['raid','giftbomb','sub'].includes(t)){if(g==='low')return'med';if(g==='med')return'high';return'high';}
  return g;
}
function triggerEffects(evType,name){
  const col=EVC[evType]||'#fff', intensity=getIntensity(evType);
  ...
  if(intensity==='high'&&fd('effectLightning')!==false)fire('ev-lightning','on',700);
  if(fd('showParticles')!==false)burstParticles(col,{low:8,med:18,high:35}[intensity]||14);
  flashCorners(col);
}
function fire(id,cls,ms){
  const el=$(id);if(!el)return;
  el.classList.remove(cls);void el.offsetWidth;el.classList.add(cls);
  setTimeout(()=>el.classList.remove(cls),ms+200);
}
function flashCorners(col){
  sv('--ev',col);
  document.querySelectorAll('.cc').forEach(c=>{c.classList.add('ev-flash');setTimeout(()=>c.classList.remove('ev-flash'),700);});
}
...
function spawnParticle(){
  ...
  const sz=rand(2,6),dur=rand(3,8),col=cols[rint(0,cols.length-1)];
  const x=rand(4,cw-4),y=rand(4,cw-4),dx=rand(-80,80),dy=-rand(50,200),dx2=rand(-35,35),dy2=-rand(25,90),op=rand(.3,.85);
  el.style.cssText=`width:${sz}px;height:${sz}px;left:${x}px;top:${y}px;background-color:${col};box-shadow:0 0 ${sz*2.2}px ${col};--dx:${dx}px;--dy:${dy}px;--dx2:${dx2}px;--dy2:${dy2}px;--op:${op};animation-duration:${dur}s;animation-delay:${rand(0,.5)}s;`;
...
function burstParticles(col,n){
  ...
  for(let i=0;i<n;i++)setTimeout(()=>{
    const el=document.createElement('div');el.className='cp';
    const sz=rand(4,11),dur=rand(1.2,3),x=rand(0,cw),y=rand(0,cw),dx=rand(-150,150),dy=rand(-180,-30);
    el.style.cssText=`width:${sz}px;height:${sz}px;left:${x}px;top:${y}px;background-color:${col};box-shadow:0 0 ${sz*3}px ${col};--dx:${dx}px;--dy:${dy}px;--dx2:${dx*.3}px;--dy2:${dy*.4}px;--op:.95;animation-duration:${dur}s;`;
    c.appendChild(el);el.addEventListener('animationend',()=>{try{el.remove();}catch(e){}});
  },i*28);
}
...
function applyAll(){
  const root=$('cam-root');
  sv('--cw',CAM_SIZE.w+'px');
  sv('--cs',CAM_SIZE.cs+'px');
  sv('--bw',CAM_SIZE.bw+'px');
  ...
  const badge=$('live-badge');if(badge)badge.classList.toggle('show',fd('showLiveBadge')!==false);
  buildCorners();
  startGoalSwitcher();
  startParticles();
}
...
window.addEventListener('onEventReceived',function(obj){
  ...
  if(L==='follower-latest'){
    live.follow=E.name||'—';if(followers!==null)followers++;
    renderGoalBar(goalMode);triggerEffects('follow',E.name);
